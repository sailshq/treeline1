...
module.exports = function(sails, socket) {
  ...
  return {
    ...
    writePacks: function(options, data, cb) {
      ...
      async.eachLimit(_.keys(data), 10, function(machineDir, nextPack) {
        ...
        if(data[machineDir].npmPackageName) {
          ...
          // Get the package version, falling back to publishedReleaseVersion key to be backwards compatible
          var packVersion = data[machineDir].version || data[machineDir].publishedReleaseVersion;
          ...
          var buildPackageJson = function() {
            ...
            packageJson.dependencies[data[machineDir].npmPackageName] = packVersion;
          };
