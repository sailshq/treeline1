...
module.exports = function runApp (conf, args, cb) {
  ...
  var projectName = (conf.targetProject.fullName);

  log();
  log(chalk.gray('Preparing to preview app: "' + chalk.bold(projectName) + '"...'));

  log.hr();
  log('Treeline preview is warming up...'.grey);
  // log();
    ...
    return {
      loadUserConfig: function(cb) {
        loadUserConfig(function(err, userConfigs) {
          ...
          // Respect the user's migrate setting
          if (!(userConfigs.models && userConfigs.models.migrate)) {
            userConfigs.models.migrate = 'alter';
          }
  ...
  watch.start(treelineCliConfig, options, function(err) {
    if (err){
      for (var i in delayedLog.timers) {
        clearTimeout(delayedLog.timers[i]);
      }
    ...
    sails.lift(liftOptions, function (err) {
      ...
      logTree();
      log();
      log('Your '+chalk.green('Treeline')+' app is now running in '+chalk.cyan('local preview mode')+'.');
