	...
	return {
		...
		reloadAllModels: function (config, options, cb) {
			...
			async.auto({
				...
				write: ['models', function(cb, results) {
			...
			buildDictionary.optional({
				...
				async.auto({
					...
					controllers: function(cb) {
						fs.readdir(path.resolve(process.cwd(), (options.export ? '' :  'node_modules/yarr/'), 'api/controllers'), function(err, files) {
					...
					writeToDisk: ['controllers', function(cb, results) {
						...
						async.forEach(Object.keys(models), function(globalId, cb) {
							...
							fs.outputFile(path.join(process.cwd(), (options.export ? '' :  'node_modules/yarr/'), '/api/models/', globalId+'.attributes.json'), json, function(err) {
								...
								fs.outputFile(path.join(process.cwd(), (options.export ? '' :  'node_modules/yarr/'), '/api/controllers/', globalId+'Controller.js'), "module.exports = {};", function(err) {
									if (err) {throw new Error(err);}
									cb();
								});
...
function clean(cb) {
	...
	glob(path.join(process.cwd(), 'node_modules/yarr/api/models/*.*'), function(err, files) {
