...
module.exports = function(sails, socket) {
	...
	return {
		...
		writeModels: function (models, options, cb) {
			...
			cb = cb || new Function();
			options = options || {};
			var config = options.config || sails.config.treeline;
			...
			buildDictionary.optional({
				...
				// Keep an array of any new models we encounter, so we can add them to Treeline
				var newModels = options.forceSync ? _.values(models) : [];

				// Loop through through the user models
				_.keys(userModels).forEach(function(userModelGlobalId) {

					// If we already know about this one in Treeline, just merge our Treeline version with the user version
					...
					// Treeline, so that we write a .json file for it
				...
				async.auto({
					...
					// Upload any new models to Treeline
...
function clean(cb) {
	...
	glob(path.join(process.cwd(), 'node_modules/treeline/api/models/*.*'), function(err, files) {
